...
<script runat="server" language="C#">
    ...
    protected void automatic_create_qcinsp()
    {
        ...
        lnk.newval.set("$qcblock$", newblock.id);
    }
    ...
    protected void insert_inspection_sample()
    {
        ...
        if (!qc.is_enough_inspection_sample(bat, blk, noinsp, loc))
        {
            ...
            result.set("stopsample", "0");
            result.set(key.SAMPLECOUNT, samplecount);
        }
        else
        {
            result.set("stopsample", "1");

            var desc = new CParameters("$errdesc$", "enough sample");
            var res = new CParameters(new string[] {
                                    "matchcode", "S016",
                                    /*"ctrls2match"*/"send", desc.get_data(false),
                                    "dlgtype", "1",
                                    "isdialog", "1"
                                });
            /*var res = new CParameters("matchcode", "S016");
            res.set("ctrls2match", desc.get_data());
            res.set("dlgtype", "1");
            res.set("isdialog", "1");*/
